<template>
    <div class="q-pa-md row justify-center" v-if="loaded">
        <q-card style="width: 1200px;" class="q-mt-lg q-pa-md">
            <q-item class="row">
                <q-item-section class="col-2">
                  <q-select style="width: 150px" outlined label-color="orange" v-model="sKey" :options="options" label="Label" />
                </q-item-section>
                <q-item-section class="col-3">
                      <q-input v-model="sValue" >
                        <template v-slot:append>
                          <q-btn flat @click="search">
                            <q-icon name="search" />
                          </q-btn>
                        </template>
                      </q-input>
                ...
                <q-item-section>
                  <div class="row justify-end">
                     <Page :current.sync="current" :total="total" :page-size="page_size"  show-elevator  show-total/>
                  </div>
                </q-item-section>
            </q-item>
          ...
          <q-list separator class="q-ma-md q-mt-md">
                <q-item>
                  ...
                  <q-item-section> <div class="text-h6 text-purple"> Index </div> </q-item-section>
                ...
                <q-item v-else v-for="(item,idx) in data" :key="idx">
                  ...
                  <q-item-section>
                    <q-item-label>
                      <q-btn style="font-size: 1rem"  dense flat rounded color="primary" :label="item.title" no-caps @click="$router.push({name:'problem',params:{'index':item.index}})"/>
                    </q-item-label>
                  </q-item-section>
                  <q-item-section>
                    <q-item-label>
                      <q-chip :color="colorForRatio(item.ratio/100)" text-color="white" dense style="font-size: 1rem;width: 7.5rem" >
                        {{item.ratio.toFixed(2)}}% ({{item.accepted_count}}/{{item.all_count}})
                      </q-chip>
                    </q-item-label>
                  </q-item-section>
                </q-item>
...
<script>
    export default {
        ...
        methods: {
            ...
            search() {
                let r = this.current*this.page_size
                let l = r - this.page_size + 1
                let form = new FormData()
                form.set("l",l)
                form.set("r",r)
                let rule = this.get_rules()
                if(rule!=null) form.set("rule",JSON.stringify(rule))
                this.$req.post("getProblemset",form)
                .then(res=>{
                    if(res.errno == undefined) {
                      this.data = res.data
                      for(let item of this.data) {
                          if (item.all_count==0) item.ratio = 100.0
                          else item.ratio = item.accepted_count/item.all_count * 100
                      }
